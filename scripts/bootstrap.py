...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.source:
            ...
            return subprocess.call([uv, 'run', '--project', str(source), '--frozen', '--no-dev', 'godot-mcp', 'install', '--source', str(source), '--godot', args.godot, *remaining])
        engine = args.engine
        if not engine:
            detected = subprocess.run([args.godot, '--version'], text=True, capture_output=True, timeout=15, check=True)
            match = re.search(r'\d+\.\d+\.\d+', detected.stdout)
            if not match:
                raise ValueError('Cannot determine Godot version. Supply --godot PATH or --engine VERSION.')
            engine = match[0]
        ...
        with tempfile.TemporaryDirectory(prefix='godot-mcp-install-') as directory:
            ...
            return subprocess.call([uv, 'run', '--project', str(source), '--frozen', '--no-dev', 'godot-mcp', 'install', '--source', str(source), '--godot', args.godot, *remaining], env=env)
