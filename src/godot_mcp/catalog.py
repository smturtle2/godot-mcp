...
RUN_REF = obj({"run_id": S, "path": {"type": "string", "pattern": "^/root(?:/|$)", "maxLength": 2048}}, ("path",))
...
VALUE = {"description": "JSON or tagged Godot value ($type, with named components). Resource uses uri, or class/properties to create an embedded engine resource in a property edit; Color space is linear or srgb."}
...
LAYOUT = obj({"preset": enum("top_left", "top_right", "bottom_left", "bottom_right", "center_left", "center_top", "center_right", "center_bottom", "center", "left_wide", "top_wide", "right_wide", "bottom_wide", "vcenter_wide", "hcenter_wide", "full_rect"), "size_flags_horizontal": integer(0, 15), "size_flags_vertical": integer(0, 15), "minimum_size": V2})
NODE_SPEC = obj({"key": {**S, "description": "Optional unique logical identifier for this record."},
                 ...
                 "name": {**S, "description": "Node name."}, "source": NODE_SOURCE, "properties": PROPS, "layout": LAYOUT},
                ("name", "source"))
...
TOOL_SPECS = [
    ...
    tool("create_nodes", "Create a flat related-node batch. Each node requires name and exactly one source choice: class, instance, or duplicate; parent_key links nodes within the batch. layout supplies Control presets/size flags/minimum size after parenting. Property Resource values may use class/properties for embedded materials/shapes; script may reference an existing script URI. Returns actual names and references.", {"parent": REF, "nodes": arr(NODE_SPEC, 1000, 1)}, ("parent", "nodes")),
    ...
    tool("wait_for_condition", "Observe scene/node/property/signal conditions until satisfied or a bounded timeout; returns last observation.", {"run_id": S, "condition": CONDITION, "timeout_ms": integer(1, 60000), "poll_ms": integer(1, 1000)}, ("condition",), read=True),
    ...
    tool("get_logs", "Read historical editor or selected-run log entries with cursor pagination. origin=runtime uses the selected run. Runs started through this connection retain editor-side output and exception stops, readable while paused or stopped. Log occurrence or silence does not establish whether current source is valid or a runtime problem is resolved.", {"run_id": S, "origin": enum("editor", "runtime"), "kinds": arr(enum("error", "warning", "log"), 3), "since": SAFE_COUNTER, "limit": integer(1, 1000)}, read=True, output=LOGS_OUTPUT),
    tool("sample_performance", "Measure supported Performance monitors over time; include units, sample count and conditions. Unknown metrics are rejected.", {"run_id": S, "duration_ms": integer(1, 60000), "metrics": arr(enum("process_ms", "physics_ms", "fps", "memory_bytes", "objects", "draw_calls", "primitives", "video_memory_bytes"), 8, 1)}, ("duration_ms", "metrics"), read=True),
    ...
    tool("stop_game", "Stop the specified run, release injected input, and confirm process termination.", {"run_id": S}, (), idempotent=True),
    tool("send_input", "Send timestamped key/mouse/touch/action events through Godot input; each event selects exactly one named kind payload. Omit run_id after run_scene/get_context to use that observed run; explicit IDs remain supported. Capture position and relative coordinates use capture pixels without caller-side conversion. Timeouts retain applied effects; retry failed observation or capture rather than repeating the mutation.", {"run_id": S, "events": arr(INPUT, 1000, 1), "capture_uri": S, "wait_for": CONDITION, "timeout_ms": integer(1, 60000), "capture_after": B, "release_after": B, "observe": arr(obj({"node": RUN_REF, "properties": arr(S, 32, 1)}, ("node", "properties")), 32, 1)}, ("events",), output=SEND_INPUT_OUTPUT),
    tool("inspect_debugger", "Read suspended DAP stack/scopes/variables; frame handles become stale on continue. GDScript is supported.", {"run_id": S, "frame_id": integer(), "pause_id": S, "variables_reference": integer()}, (), read=True),
    ...
    tool("debug_control", "Pause, continue, step over or step into GDScript. step_out reports unsupported on Godot 4.7.2.", {"run_id": S, "action": enum("pause", "continue", "step_over", "step_into", "step_out"), "pause_id": S}, ("action",)),
