@tool
extends RefCounted
## Explicit diagnostics describe captured source snapshots, independent of live edits.
...
func start(p: Dictionary) -> Dictionary:
	...
	var result: Dictionary = {"operation_id": id, "status": "pending", "basis": "snapshot", "sources": [], "state": "pending", "scope": "requested_sources" if not p.get("uris", []).is_empty() else "project_sources", "coverage": {"complete": false, "reason": "Validation is pending."}}
...
func _evaluate(p: Dictionary) -> Dictionary:
	...
	if uris.is_empty(): return {"sources": [], "basis": "snapshot", "state": "unavailable", "scope": scope, "coverage": coverage, "reason": "No authored sources are available to validate."}
	...
	var result: Dictionary = {"sources": checked.sources, "basis": "snapshot", "state": state, "scope": scope, "coverage": coverage, "observed_errors": errors, "observed_warnings": warnings}
	if fresh and coverage.complete:
		result.error_count = errors
		result.warning_count = warnings
	for key: String in ["snapshot_id"]:
