Godot MCP v4.7.2_19 · Godot 4.7.2 · Section: tools
...
Use `work` for task workflows and `model` for editor, resource, and runtime concepts. `create_project` prepares an empty directory through editor connection; `install_plugin` links an existing closed project.

For unfinished work, query its `operation_id` with `get_operation_result`; use `resume_script_changes` only for a blocked source bundle. `get_logs` reads historical errors; `get_diagnostics` explicitly validates a source snapshot. Saving, validation, and runtime behavior are separate facts, summarized in mutation receipts.
