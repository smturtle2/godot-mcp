# Validation evidence — v4.7.2_3

Validated on Linux x86_64 with Godot 4.7.2, Python 3.13, the locked official MCP SDK 2.2.0, and protocol 2026-07-28.

| Layer | Command | Result |
| --- | --- | --- |
| Contracts, transport, discovery, installation transactions | `uv run pytest -q` | 55 passed; two engine tests opt-in |
...
The clean installer check runs the installed executable without source `PYTHONPATH`, from a directory outside the Godot project. It verifies a project-independent stdio startup before linking a project, a failed connection before the editor opens, and a successful connection afterwards. Transaction unit tests verify rollback restores the active executable record, plugin, project settings, local linking record, and project index, including recovery after project installation failure. Client registration and launch are performed by the MCP client and are outside installer transactions.
...
Installer boundary tests verify that legacy app records and external settings are never accessed or modified during install, update, or rollback. Removed client-configuration flags are rejected before installation begins.
