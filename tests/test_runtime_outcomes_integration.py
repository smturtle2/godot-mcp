...
func _initialize() -> void:
    ...
    await process_frame
    var failed := await helper.send_input({"events": [{"type": "action", "action": "ui_accept", "pressed": true}], "wait_for": {"type": "property", "node": {"path": "/root"}, "property": "definitely_missing"}, "timeout_ms": 10, "capture_after": true})
    ...
    var success := await helper.send_input({"events": [{"type": "action", "action": "ui_accept", "pressed": false}]})
    print("SUCCESS=" + str(success))
    assert(success.processed == 1 and success.held_inputs == 0)
    assert(success.status == "completed" and success.complete)
    var timed := await helper.send_input({"events": [{"type": "key", "key": "Enter", "pressed": true}], "wait_for": {"type": "node", "node": {"path": "/root/Missing"}, "exists": true}, "timeout_ms": 20})
    ...
    await helper.send_input({"events": [{"type": "key", "key": "Enter", "pressed": false}]})
    var observation := await helper.wait_condition({"type": "node", "node": {"path": "/root/Missing"}, "exists": true}, 20, 5)
